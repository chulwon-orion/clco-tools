...
```
your-project/
├── .claude/hooks/clco_notify.py
├── .env.clconotify-example
└── setup_clco_notify.py
```
...
# python3 대신 python을 쓰는 경우
python setup_clco_notify.py --python python
```

설치 스크립트가 자동으로 처리하는 항목:
- `.claude/settings.json`에 clco-notify 훅 주입 (기존 설정 보존)
- `.env.clconotify-example` → `.env.clconotify` 복사
- `.gitignore`에 `.env.clconotify` 추가
